...
function writeAnItem(nIdx) {
    ...
    sIcon = _textToHtml_nonbsp(sIcon);
    var sName = _textToHtml(getItemName(nIdx));
    //WvW 2022-04-08 : Added focus change for accessibility
    var sHTML = "<p><nobr><a id=\"" + getItemId(nIdx) + "\" onfocus =\"markItem(" + nIdx + ");\" onclick=\"changeFocus();markItem(" + nIdx + ");\""
    var sAltString = "";
    if (nItemType & 2)
        sAltString = "Page";
    else
...
//WvW 2022-04-08 : Mofification to change focus
function changeFocus() {
    //Changing focus only works on hosted help as cross frame integration is blocked locally.
    //Use try/catch to degrade (ungracefully)
    try {
        var body = top.frames[1].frames[1].document.body;
        body.setAttribute("tabindex", "1");
        body.focus();
    } catch (e) {
        console.log("Trying to change focus to content pane. Failing. Error: " + e);
    }
