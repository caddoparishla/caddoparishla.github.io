...
/* Message handler for module filter listener. */
var globalSearchFilter = "ALL"; //Default value: no filter
window.addEventListener('storage', (e) => {
	//We use storage to avoid frame communication issues

	var searchFilter = localStorage.getItem('next-search-filter');

	if(searchFilter != '') {
		globalSearchFilter = searchFilter;
		//alert('New filter: '+globalSearchFilter);
		localStorage.removeItem("next-search-filter");
	}
...
function displayTopics( a_QueryResult)
{
	...
	{
		/* WvanWeelden modification Begin module filtering */
			if(globalSearchFilter == "" && !isChromeLocal()) {

			   var GISVGFilterElement = parent.frames.ftsform.document.getElementById("keywordModule");
			   var Filtervalue = GISVGFilterElement.options[GISVGFilterElement.selectedIndex].value

			} else {

				var Filtervalue = globalSearchFilter;

			}

			   var GISVGFilter = new RegExp(Filtervalue, "gi");
			   var GISVGTemp = new Array();
			   var GISVGEdit = false;

			   if(Filtervalue !== "ALL") {
					GISVGEdit = true;
					for(var i = 0;i<a_QueryResult.aTopics.length;i++) {
						if(a_QueryResult.aTopics[i].strUrl.match(GISVGFilter) != null) {/* We have a match */
							GISVGTemp.push(a_QueryResult.aTopics[i]);
						}
					}

					if(GISVGEdit) {
						/* We need to patch the ranking */
						var rank = 1;
						for(var i=0; i<GISVGTemp.length;i++) {
						GISVGTemp[i].nIndex = rank;
						rank++;
					}

					/* Set the search array */
					a_QueryResult.aTopics = GISVGTemp;
				}
			  }
			/* End WvanWeelden modification / module filtering */
		if(!gbAIRSSL)
		{
			//set num of results
		...
		for( i = (g_CurPage-1)*nMaxResult; (i < a_QueryResult.aTopics.length)&&(i<(g_CurPage*nMaxResult)); i++ )
		{
			...
			//WvanWeelden modification
			//Add highlight to PDF files
			if(szTopicURL.substr(-4).toLowerCase() == ".pdf") {
				bAddHighLight = false;
				szTopicURL += '#search=%22'+sHighlight.substring(1, sHighlight.length-1).toLowerCase()+'%22';
			}
			//End WvanWeelden modification\
